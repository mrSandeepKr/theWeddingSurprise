  ...
  const [memories, setMemories] = useState<Memory[]>([
    {
      id: 1,
      name: "Priya Sharma",
      message: "Wishing you both a lifetime of love, laughter, and endless happiness! Can't wait to celebrate with you! ❤️",
      date: "2 days ago",
      type: "message"
    },
    {
      id: 2,
      name: "Rajesh Kumar",
      message: "So excited for the big day! You two are perfect for each other. Looking forward to the sangeet night! 🕺💃",
      date: "3 days ago",
      type: "message"
    },
    {
    ...
    },
    {
      id: 4,
      name: "Michael Chen",
      message: "Congratulations to the happy couple! May your marriage be filled with all the right ingredients: a heap of love, a dash of humor, a touch of romance, and a spoonful of understanding.",
      date: "1 week ago",
      type: "message"
    }
  ]);
      ...
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="text-lg text-rose-700 max-w-2xl mx-auto mb-8">
            Share your wishes, memories, and blessings for Sandeep and Payal. Your messages mean the world to us!
          </p>
        ...
        {showForm && (
          <Card className="border-rose-200 shadow-lg mb-12">
            <CardContent className="p-8">
              <h3 className="text-xl font-semibold text-rose-800 mb-6">Share Your Wishes</h3>
        ...
        <div className="space-y-6">
          {memories.map((memory) => (
            <Card key={memory.id} className="border-rose-200 shadow-lg hover:shadow-xl transition-shadow">
              <CardContent className="p-6">
                <div className="flex items-start space-x-4">
                  <Avatar className="h-12 w-12">
                    <AvatarFallback className="bg-rose-100 text-rose-600 font-semibold">
                      {memory.name.split(' ').map(n => n[0]).join('').substring(0, 2)}
                    </AvatarFallback>
                  </Avatar>
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="font-semibold text-rose-800">{memory.name}</h4>
                      <span className="text-sm text-rose-500">{memory.date}</span>
                    </div>
                      ...
                      <div>
                        ...
                        <div className="bg-gradient-to-br from-rose-100 to-pink-100 rounded-lg p-8 flex items-center justify-center">
                          <div className="text-center text-rose-600">
                            ...
                            <p className="text-sm">Photo shared by {memory.name}</p>
                          </div>
                        </div>
                      </div>
                    ) : (
                      <p className="text-rose-700 leading-relaxed">{memory.message}</p>
                    )}
          ...
          <Card className="border-rose-200 shadow-lg bg-rose-50/50">
            <CardContent className="p-6">
              <p className="text-sm text-rose-600 italic">
                All messages and photos are moderated before appearing on the memory wall. 
                Thank you for keeping this space positive and joyful! ✨
              </p>
            </CardContent>
          </Card>
        </div>
