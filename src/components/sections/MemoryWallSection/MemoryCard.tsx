...
function MemoryImageGallery({
  ...
  return (
    <div className={`w-full min-w-0 ${isMobile ? "-mx-4" : ""}`}>
      <div
        className={`flex gap-4 overflow-x-auto scrollbar-hide pb-2 w-full ${isMobile ? "px-4" : ""}`}
      >
...
function MemoryCardAvatar({
  memory,
  isMobile,
}: {
  memory: MessageStruct;
  isMobile: boolean;
}) {
  return (
...
function MemoryCardHeader({
  ...
  return (
    <div
      className={`flex ${isMobile ? "flex-col space-y-1" : "items-center justify-between"} mb-2`}
    >
      <h4
        className={`font-semibold text-rose-800 ${isMobile ? "text-sm" : "text-base"}`}
      >
        {memory.name}
      </h4>
      <span
        className={`${isMobile ? "text-xs" : "text-sm"} text-rose-500 ${isMobile ? "" : "flex-shrink-0"}`}
      >
        {memory.date}
      </span>
    </div>
...
function MemoryCardMessage({
  ...
  return (
    <p
      className={`text-rose-700 leading-relaxed mb-3 ${isMobile ? "text-sm" : "text-base"}`}
    >
      {message}
    </p>
  );
  ...
  return (
    <Card className="border-rose-200 shadow-lg hover:shadow-xl transition-shadow overflow-hidden">
      <CardContent className={isMobile ? "p-4" : "p-6"}>
        <div
          className={`flex items-start ${isMobile ? "space-x-3" : "space-x-4"} min-w-0`}
        >
