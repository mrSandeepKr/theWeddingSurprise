...
const MemoryWallSection = lazy(() => 
  import("@/components/sections/MemoryWallSection").then(module => ({
    default: module.default
  }))
);
...
function SectionLoader({ height = "h-96" }: { height?: string }) {
  return (
    <div className={`${height} flex items-center justify-center bg-gradient-to-br from-rose-50 to-pink-50`}>
      <div className="text-center">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-rose-600 mx-auto mb-4"></div>
        <p className="text-rose-600 font-medium">Loading beautiful memories...</p>
      </div>
    </div>
  );
}
